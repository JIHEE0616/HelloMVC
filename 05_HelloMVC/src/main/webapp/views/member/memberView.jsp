...
<section id=enroll-container>
		<h2>회원 정보 수정</h2>
		<form action="<%=request.getContextPath()%>/member/updateMemer.do" id="memberFrm" method="post" >
			<table>
				<tr>
					<th>아이디</th>
					<td>
				...
				<tr>
					...
					<td>
						<!-- DB정보에 따라 분기처리할것 -->
							...
							<%=m.getGender()=='M'?"checked":"" %>>  <!-- 삼항연산자 -->
							<label for="gender0">남</label>
							
							<input type="radio" name="gender" id="gender1" value="F"
							<% if(m.getGender()=='F'){ %> checked <%} %>> <!-- if문 -->
			...
			<input type="submit" value="정보수정" onclick="fn_updateMember();"/>
	...
	<script>
		...
		const fn_updatePassword=()=>{
			//패스워드수정패이지 새창으로 연결
	        open("<%=request.getContextPath()%>/member/updatePassword.do?userId=<%=loginMember!=null?loginMember.getUserId():""%>","_blank","width=400, height=210,left=500 ,top=200");
	     }
	</script>
